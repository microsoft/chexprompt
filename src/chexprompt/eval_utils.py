...
def parse_completion_tuples_to_dict(completion_text: str) -> Dict[str, int] | None:
    ...
    completion_text = completion_text.replace("),", "::").replace("(","").replace(")","").replace("[","").replace("]","")
    completion_text = completion_text.split("::")
    
    try:
        for pair in completion_text:
            pair = pair.strip().split(",")
            error_key = ERROR_TYPE_MAP[pair[0].strip()] if pair[0].strip() in ERROR_TYPE_MAP else pair[0].strip()
...
def parse_rating_text(rating_text: str) -> Tuple[Dict[str, int], Dict[str, int]]:
    ...
    def _get_significant_insignificant_tuple_lists(rating_text: str) -> Tuple[str, str]:
        ...
        insignificant = rating_text.split("\nNumber of clinically insignificant errors by type:")[-1].strip()

        significant = rating_text.split("\nNumber of clinically insignificant errors by type:")[0]\
                                            .split("Number of clinically significant errors by type: ")[-1].strip()
        
        return significant, insignificant
    
    rating_text = _check_remove_explanations(rating_text)

    significant_seq, insignificant_seq = _get_significant_insignificant_tuple_lists(rating_text)
...
def extract_rating_dicts(completion: Dict[str, str]) -> Tuple[Dict[str, int], Dict[str, int]]:
